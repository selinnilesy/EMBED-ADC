...
unsigned char save_var; // i use this to save portd 7-segment when heater is on it flickers somehow
...
void LCDDat(unsigned char dat) {
  ...
  if(PORTAbits.RA5==1) {flag=1; PORTAbits.RA5=0;}
  LCD_EN = 0;
  LCD_RS = 1;
  save_var = PORTD;
  LCD_PORT = dat;
  ...
  LCD_RS = 0;
  PORTD = save_var;
  if (flag) { flag=0; PORTAbits.RA5=1;}
